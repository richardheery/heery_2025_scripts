...
plot_tmr_regions = function(tmrs, transcript_regions_gr, regions_filter, normalize = T, title = "Distribution of TMRs"){
  ...
  # Get total size of each class of region
  ...
  overlaps_summary = data.frame(tidyr::complete(tibble(overlaps_summary), direction, region, fill = list(count = 0)))
  ...
  regions_filter[grep("TSS|TES", regions_filter)] = stringr::str_replace_all(regions_filter[grep("TSS|TES", regions_filter)], 
    "(?<=[+-])\\d+", \(x) as.character(as.integer(x) - 250))
  ...
  tmr_overlaps_count_filtered$region[grep("TSS|TES", tmr_overlaps_count_filtered$region)] = 
    stringr::str_replace_all(tmr_overlaps_count_filtered$region[grep("TSS|TES", tmr_overlaps_count_filtered$region)], "(?<=[+-])\\d+", \(x) as.character(as.integer(x) - 250))
